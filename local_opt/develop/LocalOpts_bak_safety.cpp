						...
						if (op == Instruction::Mul) {
							...
							if (!isa<Constant>(L) ) {
								...
								unsigned op2 = LI->getOpcode();
										if  (LI->getNumOperands() == 2 && op2 == Instruction::Mul) {
											...
											errs() << "wo yao mei zi6" << "\n";
												if (ConstantInt *Res = calcOpRes(Instruction::Mul, dyn_cast<ConstantInt>(LR), dyn_cast<ConstantInt>(R))) {
											errs() << "wo yao mei zi7" << "\n";
													L = LL;
													R = Res;
												}
