	...
	class LocalOpts : public FunctionPass {
		public:
			...
			bool subHelper(Value *L, Value *R, Value *&res, int num, OptSummary *ops, BasicBlock::iterator &ii) {
				...
				Value *X = 0, *Y = 0, *Z = R, *V;
				errs() << "flag&&&&&&&&&&&&&&&&&&&&&&&&&&" << "\n";
				if (num && match(L, m_Add(m_Value(X), m_Value(Y)))) {
					errs() << "flag000000000000000"<< "\n";
					if (subHelper(Y, Z, V, num-1, ops, ii)) {
						//if (V == NULL) {
					//		errs() << "Yes" << "\n";
					//	}
					//	errs() << "\n" << "V's value" << cast<ConstantInt>(V)->getValue()<<"\n";
					//	errs() << "..............fsfsaf...." << (V->getType() == X->getType()) << "\n";
						//Instruction *newInst;
						//Twine *Name = new Twine();
						//BinaryOperator *newInst= BinaryOperator::Create(Instruction::Add, X, *(&V), *Name, ii);
						
						BinaryOperator *newInst= BinaryOperator::Create(Instruction::Add, X, ConstantInt::get(X->getType(),2));
						//if (newInst == NULL) {
					//		errs() << "^(*&*(&))*)*))" << "\n";
					//	}

						//errs() << "wo cao:" << newInst->getNumOperands() << "\n";
